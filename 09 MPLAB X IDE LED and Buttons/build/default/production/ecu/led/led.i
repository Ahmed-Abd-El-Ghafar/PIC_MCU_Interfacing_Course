...
ret_status led_turn_on(led_t led){
    gpio_pin_direction_init(led.port_name, led.pin, 0X00);
...
# 55 "ecu/led/led.c"
ret_status led_sequence (led_t leds[], uint8_t seq[][8], uint8_t seq_len){
    uint8_t l_counter = 0x0U, l_leds_count = 0x0U;
    for(l_counter = 0x0U; l_counter<seq_len; ++l_counter){
        for(l_leds_count = 0x0U; l_leds_count<8; ++l_leds_count){
