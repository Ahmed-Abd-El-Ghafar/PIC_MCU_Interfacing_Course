...
# 1 "mcal/gpio/../../mcal/types.h" 1
# 11 "mcal/gpio/../../mcal/types.h"
...
ret_status gpio_default_init(uint8_t port_numbers){
    ret_status ret = R_OK;
    uint8_t l_counter = 0x0U;
    for(l_counter=0x0U; l_counter<0x05 -1; l_counter++){
        *tris_reg[l_counter] = 0XFF;
    }
    return ret;
}
# 35 "mcal/gpio/mcal_gpio.c"
ret_status gpio_pin_direction_init(port_index index, uint8_t pin, uint8_t pin_direction){
...
ret_status gpio_write_port_value(port_index index, uint8_t value){
    ret_status ret = R_OK;
    gpio_port_direction_init(index, 0X00);
    *lat_reg[index] = value;
    return R_OK;
}
